...
CREATE TABLE `user_persona` (
    ...
    `preferred_name` VARCHAR(50) DEFAULT NULL COMMENT '称呼/姓名',
    `location` VARCHAR(100) DEFAULT NULL COMMENT '所在地',
    `interests` VARCHAR(500) DEFAULT NULL COMMENT '兴趣/话题偏好',
    `tone` VARCHAR(200) DEFAULT NULL COMMENT '语气要求',
    `custom_instructions` TEXT DEFAULT NULL COMMENT '自定义指令/相处模式',
    `source_type` VARCHAR(32) NOT NULL DEFAULT 'UNKNOWN' COMMENT '来源类型',
...
CREATE TABLE `diary` (
    ...
    `source_revision` BIGINT DEFAULT NULL COMMENT '来源内容单调版本号',
...
DROP TABLE IF EXISTS `soul_connection_event`;

CREATE TABLE `soul_connection_event` (
...
DROP TABLE IF EXISTS `product_event_scope`;

CREATE TABLE `product_event_scope` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '数据库主键',
...
CREATE TABLE `embedding_task` (
    ...
    UNIQUE KEY `uk_embedding_task_execution` (`task_execution_id`),
...
CREATE TABLE `life_graph_entity` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `user_id` VARCHAR(64) NOT NULL COMMENT '用户ID',
    `type` VARCHAR(32) NOT NULL COMMENT '实体类型: Person/Event/Place/Work/Emotion/Topic/Item/User',
    ...
    `confidence` DOUBLE NOT NULL DEFAULT 0.5 COMMENT '可信度，范围 0 到 1',
    `importance` DOUBLE NOT NULL DEFAULT 0.5 COMMENT '长期生活记忆价值，范围 0 到 1',
    `match_allowed` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否允许参与匹配',
    `hidden` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否隐藏',
...
CREATE TABLE `life_graph_relation` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `user_id` VARCHAR(64) NOT NULL COMMENT '用户ID',
    `source_id` BIGINT NOT NULL COMMENT '物理图边端点实体ID（用于兼容无向邻接）',
    `target_id` BIGINT NOT NULL COMMENT '物理图边端点实体ID（用于兼容无向邻接）',
    `semantic_source_id` BIGINT DEFAULT NULL COMMENT '语义关系的主语实体ID',
    `semantic_target_id` BIGINT DEFAULT NULL COMMENT '语义关系的宾语实体ID',
    `type` VARCHAR(64) NOT NULL COMMENT '关系类型: PARTNER_OF/LIKES/HAPPENED_AT/TRIGGERED/PARTICIPATED_IN 等',
    ...
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_life_graph_relation_user_semantic_edge` (
        `user_id`,
        `semantic_source_id`,
        `semantic_target_id`,
        `type`
    ...
    KEY `idx_life_graph_relation_user_semantic_source` (`user_id`, `semantic_source_id`),
    KEY `idx_life_graph_relation_user_semantic_target` (`user_id`, `semantic_target_id`),
...
CREATE TABLE `life_graph_relation_evidence` (
    ...
    `source_type` VARCHAR(32) NOT NULL COMMENT '来源类型：DIARY/PLAZA/LEGACY',
...
DROP TABLE IF EXISTS `life_graph_entity_evidence`;
...
CREATE TABLE `life_graph_task` (
    ...
    UNIQUE KEY `uk_life_graph_task_execution` (`task_execution_id`),
...
DROP TABLE IF EXISTS `agent_tool_trace`;
...
CREATE TABLE `developer_config` (
    ...
    `scopes` VARCHAR(512) NOT NULL DEFAULT 'MEMORY_READ' COMMENT 'API Key capability scopes',
    `revoked_at` DATETIME DEFAULT NULL COMMENT 'API Key revoke timestamp',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
...
DROP TABLE IF EXISTS `web_access_policy`;
...
DROP TABLE IF EXISTS `soul_report`;
...
DROP TABLE IF EXISTS `cognitive_conflict`;

CREATE TABLE `cognitive_conflict` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `user_id` VARCHAR(64) NOT NULL COMMENT '用户ID',
...
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT '账号注销请求台账（受限、可重试）';
