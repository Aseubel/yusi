...
| ID | 功能特性 | 描述 | 优先级 |
|:---|:---|:---|:---|
| **F8.1** | Agent 人格系统 | Agent 拥有稳定的人格配置（语气、风格、边界感），用户可感知其"性格"且可微调 | P0 |
| **F8.2** | 主动问候与洞察 | Agent 基于用户近期状态（mid-memory），在合适时机主动发起对话（如"最近似乎..."） | P0 |
...
| ID | 功能特性 | 描述 | 优先级 |
|:---|:---|:---|:---|
| **F9.1** | 匹配后引导 | 匹配成功后，Agent 提供破冰引导（话题建议、情景游戏推荐），而非直接抛给用户 | P0 |
| **F9.2** | 共鸣信号 | 广场内容支持对特定用户匿名发送"共鸣信号"（非匹配渠道的轻量连接） | P1 |
| **F9.3** | 情景室匹配入口 | 匹配成功的双方可一键创建私人情景室，通过结构化情景破冰 | P1 |
| **F9.4** | 群体共鸣发现 | Agent 从广场中发现"正在经历相似阶段"的 3-5 人小群体，提供匿名小群连接 | P2 |
| **F9.5** | 匹配反馈循环 | 匹配结果（接受/跳过/匹配成功后的互动质量）反馈至 Agent 认知层，优化后续推荐 | P0 |
...
| ID | 功能特性 | 描述 | 优先级 |
|:---|:---|:---|:---|
| **F11.1** | mid-memory 持久化 | 将 mid-memory 从"服务内存"升级为持久化的 MidMemorySnapshot（表结构已有设计），支持跨重启保持 | P0 |
| **F11.2** | 记忆衰减机制 | mid-memory 支持时间衰减（valid_until），过期自动降低权重，不参与匹配和对话上下文 | P0 |
...
### Phase 1: Agent 觉醒（核心）— 预计 3-4 周

1. **F11.1 mid-memory 持久化**（P0 基础依赖）
2. **F11.2 记忆衰减机制**（P0 基础依赖）
3. **F8.1 Agent 人格系统**（P0）
4. **F8.2 主动问候与洞察**（P0）
5. 重构 `ChatMemoryStore` 注入 persona context
6. 重构 `MatchServiceImpl.getMatchStatus()` 修复 stale 文案

**Phase 1 交付物**：用户能感受到 Agent "有性格"且"会主动关心自己"

### Phase 2: 连接深化 — 预计 2-3 周
...
| 问题 | 位置 | 修复 |
|:---|:---|:---|
| `getMatchStatus().nextMatchTime` 写死"每日凌晨 2:00" | `MatchServiceImpl.java:689` | 改为动态计算：从 `@Scheduled` cron 解析出下次执行时间 |
| `EmotionPlazaCognitionIngestService` 缺失 | 新建 | 参照 `DiaryCognitionIngestEvent` 的发布方式，在广场投递成功后发布事件 |
| `runDailyMatching()` 方法名过时 | `MatchServiceImpl.java:82` | 重命名为 `runWeeklyMatching()` |
| Prompt 散落各处 | `MatchAssistant.java` 等 | 统一迁移到 `PromptManager` 或 prompt 模板表管理 |
