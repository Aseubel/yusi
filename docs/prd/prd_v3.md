...
| ID | 功能特性 | 描述 | 优先级 | 状态 |
| :--- | :--- | :--- | :--- | :--- |
| **F5.1** | 日记地理关联 | 支持写日记时添加地理位置（自动定位/地图选点/搜索POI），将故事锚定在地图上。 | P0 | [待开发] |
| **F5.2** | 常用与重要地点 | 用户可管理"常用地点"（家、公司）及定义"重要地点"（如：初恋的地方、常去的咖啡馆），辅助实体关系构建与消歧。 | P0 | [待开发] |
| **F5.3** | 足迹地图 | 在地图上可视化的展示用户日记足迹，支持按时间、地点、情感维度聚类查看过往记忆。 | P1 | [待开发] |
