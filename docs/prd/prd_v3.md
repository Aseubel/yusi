...
| ID       | 功能特性       | 描述                                                              | 优先级 | 状态     |
| :------- | :------------- | :---------------------------------------------------------------- | :----- | :------- |
| **F8.1** | 记忆分层存储   | 短期记忆（会话级）、中期记忆（近期重要事件）、长期记忆（人生图谱） | P0     | ✅ 已完成 |
| **F8.2** | 记忆检索增强   | 基于用户问题动态检索相关记忆，融合图谱 + 向量双重检索               | P0     | ✅ 已完成 |
| **F8.3** | 记忆压缩摘要   | 对历史对话进行智能压缩，保留关键信息，降低 Token 消耗             | P1     | ✅ 已完成 |
| **F8.4** | 记忆遗忘机制   | 模拟人类遗忘曲线，自动衰减不重要记忆，强化重要记忆               | P1     | ✅ 已完成 |
| **F8.5** | 对话上下文感知 | AI 在对话中主动引用用户过往经历，实现"我记得你说过..."           | P2     | ✅ 已完成 |
| **F8.6** | MCP 记忆搜索   | 通过 MCP 协议提供统一的记忆搜索接口，整合中期记忆与短期上下文      | P1     | ✅ 已完成 |
...
| ID       | 功能特性     | 描述                                                              | 优先级 | 状态     |
| :------- | :----------- | :---------------------------------------------------------------- | :----- | :------- |
| **F9.1** | 富文本编辑器 | 支持粗体、斜体、下划线、标题、列表、引用等格式                    | P0     | ✅ 已完成 |
| **F9.2** | 图片插入     | 支持在日记中插入图片                     | P1     | 📋 待开发 |
...
| 功能         | 描述                                           | 优先级 | 状态     |
| :----------- | :--------------------------------------------- | :----- | :------- |
| 内容草稿     | 支持聊天或日记编辑的内容自动本地保存，避免刷新丢失               | P2     | ✅ 已完成 |
| 多端同步     | 提供 PWA，实现日记、聊天记录的多端同步         | P2     | ✅ 已完成 |
...
| :------- | :--------------------------- | :--------- | :----------- | :----------- |
| **短期** | 当前会话对话历史             | 会话结束   | Redis + LangChain4j MessageWindowChatMemory | 自动维护，默认 50 条消息窗口 |
| **中期** | 近期重要事件摘要             | 30 天       | MySQL + Milvus 向量检索 | 定时任务扫描，满足时间间隔 (1h) 和消息数量 (50 条) 时触发 AI 总结 |
| **长期** | 人生图谱、关键记忆节点       | 永久       | MySQL + 图谱 | 从日记和中期记忆中抽取实体关系 |

**中期记忆总结流程**：
1. 定时任务每 5 分钟扫描一次
2. 检查用户最后一次对话时间是否超过 1 小时
3. 统计未总结的消息数量是否达到上下文窗口阈值 (50 条)
4. 满足条件时，调用 AI 对消息窗口进行总结，生成中期记忆
5. 标记已总结的消息，记录总结时间

### 4.4 MCP 记忆搜索架构

**MCP (Model Context Protocol)** 提供统一的记忆搜索接口，使 LLM 能够访问用户的记忆数据：
...
**memorySearch 接口特性**：
- **综合检索**：同时返回中期记忆（AI 总结）和短期记忆（原始对话）
- **相关度评分**：为每个结果计算相关度分数 (0-1)
- **类型标识**：区分 MID_TERM_MEMORY 和 SHORT_TERM_CONTEXT
- **可配置窗口**：支持指定最大返回结果数量
...
| **记忆框架** | LangChain4j       | 提供短期记忆上下文管理     |
| **MCP 协议** | 自定义 gRPC       | 连接 LLM 与后端记忆系统     |
