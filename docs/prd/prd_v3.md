...
### Epic 5: "人生图谱" (Life Graph) - 基于 GraphRAG
**PM目标**: 利用知识图谱技术，让 AI 真正理解用户的人生故事，而非仅做语义检索。

| ID | 功能特性 | 描述 | 优先级 | 状态 |
| :--- | :--- | :--- | :--- | :--- |
| **F5.1** | 实体自动抽取 | AI 从日记中自动提取关键实体（人物、地点、事件、情绪、物品等），构建用户专属知识图谱。 | P0 | [待开发] |
| **F5.2** | 关系图谱构建 | 基于实体间的共现和语义分析，自动推断并建立关系边（如：与某人的关系、事件的因果链）。 | P0 | [待开发] |
| **F5.3** | 多跳推理问答 | AI 可回答复杂问题，如"我和小明是怎么认识的？后来发生了什么？"，通过图谱遍历进行多跳推理。 | P0 | [待开发] |
| **F5.4** | 人生时间线 | 可视化展示用户人生重要节点的时间线，基于图谱自动聚类生成"人生章节"。 | P1 | [待开发] |
| **F5.5** | 社区洞察 | 通过图聚类算法（Leiden/Louvain），识别用户生活中的"社区"（如：工作圈、朋友圈、家庭），分别理解不同圈层的故事。 | P1 | [待开发] |
| **F5.6** | 情感图谱 | 追踪用户情绪随时间和事件的变化，识别情绪触发点（如"每次提到某人情绪都变差"）。 | P2 | [待开发] |

#### GraphRAG 技术细节

**与传统 RAG 的对比**:

| 对比维度 | 传统 RAG (v2.0) | GraphRAG (v3.0) |
| :--- | :--- | :--- |
| 数据表示 | 文本块 → 向量 | 文本 → 实体+关系 → 图谱 |
| 检索方式 | 语义相似度 Top-K | 图遍历 + 社区摘要 |
| 推理能力 | 单跳（相似片段） | 多跳（关系链推理） |
| 全局理解 | ❌ 无法总结全局 | ✅ 社区摘要支持全局问答 |
| 适用场景 | "我昨天写了什么" | "我和小明的关系是怎么变化的" |

**索引流程**:
```
用户日记 → LLM实体抽取 → 图谱构建（Neo4j/自建）
              ↓                    ↓
         向量嵌入存储          图聚类(Leiden算法)
              ↓                    ↓
         Milvus向量库          社区摘要生成
...
**查询模式**:
- **Local Search**: 针对特定实体的精确查询（如："小明是谁？"）
- **Global Search**: 需要跨多个日记综合推理（如："我的大学生活是怎样的？"）

### Epic 6: "灵魂伴侣 2.0" (Evolved AI Companion)
**PM目标**: 将 AI 从"分析工具"升级为具有个性与成长的"灵魂伴侣"。

| ID | 功能特性 | 描述 | 优先级 | 状态 |
| :--- | :--- | :--- | :--- | :--- |
| **F6.1** | 伴侣人设 | 用户可选择或自定义 AI 伴侣的性格基调（如：温暖治愈型、理性分析型、幽默调侃型等），AI 将以此风格进行所有互动。 | P0 | [待开发] |
| **F6.2** | 记忆殿堂 | AI 拥有可视化的"记忆殿堂"，基于人生图谱展示与用户的互动时间线、关键记忆节点、情感变化曲线。用户可浏览、标记或删除特定记忆。 | P0 | [待开发] |
| **F6.3** | 成长轨迹 | AI 伴侣拥有"成长值"系统，随互动深度提升解锁新能力（如：主动关怀、周期性情感总结、节日祝福等）。 | P1 | [待开发] |
| **F6.4** | 主动关怀 | AI 基于情感图谱的变化趋势，在检测到持续负面情绪时主动发起温暖对话，而非被动等待用户。支持设置免打扰时段。 | P1 | [待开发] |
| **F6.5** | 周期报告 | AI 每周/每月自动生成"灵魂报告"，基于图谱分析生成情绪波动图、关键事件回顾、人际关系变化、成长建议等。 | P2 | [待开发] |
| **F6.6** | 纪念日提醒 | AI 从图谱中识别重要纪念日（生日、纪念日、重要事件周年），主动送上关怀与回忆。 | P2 | [待开发] |

### Epic 7: "场景工坊" (Scenario Workshop)
**PM目标**: 构建 UGC 创作者生态，让社区内容自我生长。

| ID | 功能特性 | 描述 | 优先级 | 状态 |
| :--- | :--- | :--- | :--- | :--- |
| **F7.1** | 场景创作器 | 提供可视化场景创作工具，用户可定义场景标题、背景描述、预设选项、分析维度等。支持富文本和配图。 | P0 | [待开发] |
| **F7.2** | 创作者认证 | 优质创作者可申请认证，获得专属标识。认证场景优先展示，创作者可获得"灵感值"奖励。 | P1 | [待开发] |
| **F7.3** | 场景评分系统 | 用户完成场景后可对场景进行评分与评价，高分场景进入"精选榜单"。 | P1 | [待开发] |
| **F7.4** | 场景系列 | 创作者可创建"场景系列"，形成有主题的场景合集（如：职场困境系列、亲密关系系列）。 | P2 | [待开发] |
| **F7.5** | 创作激励 | 引入"灵感值"虚拟货币，用户使用场景时消耗灵感值，创作者获得对应收益。可用于解锁高级功能。 | P2 | [待开发] |

### Epic 8: "灵魂部落" (Soul Tribe)
**PM目标**: 超越 1v1 匹配，构建志趣相投的小群体社交。

| ID | 功能特性 | 描述 | 优先级 | 状态 |
| :--- | :--- | :--- | :--- | :--- |
| **F8.1** | 部落创建 | 用户可创建"灵魂部落"（3-20人），设定部落主题（如：夜猫子互助会、社恐康复联盟）。 | P0 | [待开发] |
| **F8.2** | 部落匹配 | 系统基于用户的人生图谱画像，推荐适合加入的部落。用户可申请加入，部落成员投票决定是否接纳。 | P1 | [待开发] |
| **F8.3** | 部落情景室 | 部落成员可发起专属情景室活动，增强群体凝聚力。 | P1 | [待开发] |
| **F8.4** | 匿名树洞 | 部落内设"匿名树洞"，成员可匿名倾诉，其他成员匿名回应支持。 | P2 | [待开发] |
| **F8.5** | 部落成长 | 部落拥有等级系统，随活跃度提升可解锁更多功能（增加人数上限、专属场景等）。 | P2 | [待开发] |
