...
**默认密钥模式**:
- 服务端统一使用部署环境变量提供的 AES-256-GCM 密钥加密内容（密钥不入库、不下发前端、不在网络传输）
- 客户端提交明文，服务端加密后落库；读取时服务端解密后返回
- 安全性依赖服务器与密钥管理（适合希望“无需解锁/可用 AI”的用户）

**自定义密钥模式**:
- 用户在客户端设置密码，使用 PBKDF2(SHA-256) 派生 256-bit 数据密钥
- 加密算法采用 AES-256-GCM，加密操作完全在浏览器端完成；密文传输至服务端存储
- 无备份：服务端无法解密（最高隐私），因此 RAG/AI 分析不可用
- 有备份：客户端用服务端 RSA 公钥（OAEP SHA-256）加密“派生得到的 32 字节数据密钥”后上传，网络中不会出现明文密钥；服务端仅在需要 AI/RAG 时用 RSA 私钥解封装得到数据密钥进行解密
...
后端使用统一配置前缀 `yusi.security.crypto` 管理加密所需密钥，配置来源为环境变量注入（生产环境建议由 Secret 管理系统提供）。

必需环境变量：
- `YUSI_ENCRYPTION_KEY`：Base64 的 32 字节 AES-256 密钥（DEFAULT 模式内容加解密）
- `YUSI_BACKUP_RSA_PUBLIC_KEY_SPKI_BASE64`：RSA 公钥（SPKI Base64，用于前端加密备份密钥）
- `YUSI_BACKUP_RSA_PRIVATE_KEY_PKCS8_BASE64`：RSA 私钥（PKCS8 Base64，用于服务端解密备份密钥）
