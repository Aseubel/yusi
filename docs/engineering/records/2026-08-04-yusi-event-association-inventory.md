...
| 匹配推荐 | `SoulMatch.id`，当前作为 `matchId`；`MatchProfile.version` | 推荐的是哪一对用户、当前双方动作和画像版本 | 推荐生成的 `runId` 或事件 ID；独立的 `connectionId` |
| 匹配反馈 | `MatchFeedback.id`、`matchId`、`userId`、`action` | 用户对哪条匹配做了什么反馈 | `connectionId`；反馈所对应的产品事件 ID或请求 ID |
| 灵魂聊天 | `SoulMessage.id`、`matchId`、发送方和接收方 | 消息属于哪条匹配 | 独立 `connectionId`；消息发送请求的关联 ID |
...
| Connection ID | 一次匹配后的连接关系 | 后续生成 | 从接受/开始互动到结束或阻断 |
...
- 保留 `matchId` 表示一次推荐记录。
- 双方接受并进入互动时创建 `connectionId`，后续反馈、消息、结束、举报和拉黑都使用它。
- 匹配推荐事件可记录生成它的 `runId`、画像版本和低敏理由数量，但不把敏感原文放入客户端事件。
...
1. 先完成 P0 事件 ID和两类异步任务的关联字段。
2. 为任务创建、重试和恢复补充单元测试，验证同一任务全程保留 `triggerEventId`。
3. 在服务端 Trace 开始前，再实现 P1 的连接模型和聊天记忆 `runId`，避免一次引入过多表结构。
