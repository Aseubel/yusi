...
5. **统一模型控制面、能力专用客户端**：模型管理中心负责 endpoint、capability、路由、密钥和热更新；Chat、Embedding、Speech-to-Text 分别由专用 adapter 创建。ASR 由 `service/ai/asr` 下的 OpenAI 与 DashScope adapter 处理，不能直接复用 `ChatModel`，但可以复用同一套 endpoint 配置和故障转移边界。
